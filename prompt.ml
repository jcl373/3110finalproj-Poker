...
let rec request_choice max_wager (gametable : Table.table) round (p : Table.person) : unit =
  ...
  then let bot_bet = bot_choice p max_wager in
    match bot_bet with 
    | Fold -> p.position <- Some Folded; print_choice bot_bet p; Unix.sleepf 2.; draw_player p; moveto ((fst p.location)-35) ((snd p.location)-20); draw_string (string_of_choice bot_bet p); Table.next_br_prep gametable 
    | AllIn x -> p.position <- Some (AllIn round); print_choice bot_bet p; Unix.sleepf 2.; draw_player p; moveto ((fst p.location)-35) ((snd p.location)-20); draw_string (string_of_choice bot_bet p);
    | _ -> 
      ...
      Unix.sleepf 2.;
      draw_player p; moveto ((fst p.location)-35) ((snd p.location)-20); draw_string (string_of_choice bot_bet p);
      ...
      begin
        try
          begin 
            ...
            | Fold -> p.position <- Some Folded; draw_player p; moveto ((fst p.location)-35) ((snd p.location)-20); draw_string (string_of_choice player_bet p); Table.next_br_prep gametable 
            | AllIn x -> p.position <- Some (AllIn round); draw_player p; moveto ((fst p.location)-35) ((snd p.location)-20); draw_string (string_of_choice player_bet p);
            | _ ->
              ...
              draw_player p; moveto ((fst p.location)-35) ((snd p.location)-20); draw_string (string_of_choice player_bet p);
